function [outputfinal]=ITCFit2(fname,V0,SyC,T,npd,startka,startn,corrected,ptype)
%% ITC Data fitting %%
...
x=0.95*mean(xmt);
y1=1.75*mean(ndh);
text(x,y1,{['K_{d} = (' sprintf('%5.2f',Kd*1e6) ' \pm ' sprintf('%5.2f',erKd*1e6) ' ) x 10^{-6} M'],['n = ' sprintf('%5.2f',fitresult.n) ' \pm ' sprintf('%5.2f',std(3)) ],['\Delta' 'H = ' sprintf('%5.2f',fitresult.dH*1e-3) ' \pm ' sprintf('%5.2f',std(2)*1e-3) ' kcal/mol'],['\Delta' 'S = ' sprintf('%5.2f',dS) ' cal/mol']},'EdgeColor','red');
hold off;
